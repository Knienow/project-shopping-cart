...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async (event) => {
    const getData = event.target.parentNode;
    const getProductID = getIdFromProduct(getData);
    const dataProduct = await fetchProduct(getProductID);
    const elementProduct = createCartProductElement(dataProduct);
    document.querySelector('.cart__products').appendChild(elementProduct);
    saveCartID(getProductID);
  });
